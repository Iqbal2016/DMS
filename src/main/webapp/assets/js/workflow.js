...
function showMessage(data) { 
	...
	if(data.outcome == 'success'){
		ShowSuccessMsg('Workflow saved', data.message);
...
var addBlock = function(){
	...
	'<div class="wf-block panel panel-light-grey" data-id="' + id + '">' +
		...
		'<div id="wf_block' + id + '" class="panel-collapse collapse in">' +
			'<div class="panel-body">' +
				'<button class="btn btn-md btn-red btn-o pull-right" onclick="delBlock(this);"><span class="fa fa-close"> Delete</span></button>' +
				'<div class="row">' +
					'<div class="col-md-4">' +
						'<div class="row">'+
							'<div class="col-md-auto">' +
								'<label class="control-label doa-type-label">DOA Type </label>' +
							'</div>'+
							'<div class="col-md-auto">' +
								'<label class="control-label doa-type-text"></label>' +
								'<input type="hidden" class="form-control doa-type-text-value" name="doa_type_text[]" />' +
							'</div>'+
						'</div>'+
					...
					'<div class="col-md-3">' +
						'<div class="row">'+
							'<div class="col-md-auto">' +
								'<label class="control-label doa-limit-label">Limit </label>' +
							'</div>'+
							'<div class="col-md-auto">' +
								'<label class="control-label doa-limit-text"></label>' +
								'<input type="hidden" class="form-control doa-limit-text-value" name="doa_limit_text[]" />' +
							'</div>'+
						'</div>'+
					...
					'<div class="col-md-3">' +
						'<div class="row">'+
							'<div class="col-md-auto">' +
								'<label class="control-label tat-limit-label">TAT </label>' +
							'</div>'+
							'<div class="col-md-auto">' +
								'<label class="control-label tat-limit-text"></label>' +
								'<input type="hidden" class="form-control tat-limit-text-value" name="tat_limit_text[]" />' +
							'</div>'+
						'</div>'+
						'<input type="number" value="" class="form-control tat-limit" readonly="readonly" />' +
					'</div>' +
				'</div>' +
	...
	'<tr>' +
		...
		'<td>' + 
			...
			'<input type="hidden" name="doa_type_text[]" class="doa-type-value" value="" />' +
			'<input type="hidden" name="doa_limit_text[]" class="doa-limit-value" value="" />' +
			...
			'<input type="hidden" name="tat_limit[]" class="tat-limit" value="" />' +
		'</td>' +
	'</tr>';
	...
	'<tr>' +
		...
		'<td>' + 
			...
			'<input type="hidden" name="doa_type_text[]" class="doa-type-label-value" value="" />' +
			'<input type="hidden" name="doa_limit_text[]" class="doa-limit-label-value" value="" />' +
...
$("#btnOk").on("click", function(){
	...
	var tat = $("#modal_estimated_time").val();  //alert($(this).closest('.tat-limit').val(tat))
	//$(".tat-limit").val(tat); 
	//$(this).closest('.row').find('.tat-limit').val(tat);
	// $(this).closest('.tat-limit').val(tat);
	...
	var doaType = $(".current-block").find(".doa-limit-value").val(); 
	var doaLimit = $(".current-block").find(".doa-limit-value").val();
	...
	$(".current-block .wf-step-list tr.current-row").html(
		...
		'<td>' + 
			...
			'<input type="hidden" name="doa_type_text[]" class="doa-type-value" value="' + doaType + '" />' +
			'<input type="hidden" name="doa_limit_text[]" class="doa-limit-value" value="' + doaLimit + '" />' +
...
$("#btnCancel,  #btnModalClose").on("click", function(){
	$(".wf-block").removeClass("current-block");
	$(".wf-step-list tr").removeClass("current-row");
...
$('#suite_code').on('change', function(){
	...
	LoadMainContent("tna/wf/workflowmanager/create/?suite_code=" + newSuiteCode);
});

$('#module_code').on('change', function(){
	var newSuiteCode = $("#suite_code").val();
	var newModuleCode = $("#module_code").val();
	LoadMainContent("tna/wf/workflowmanager/create/?suite_code=" + newSuiteCode + "&" + "module_code=" + newModuleCode);
});

$("#btnObjectType").on("click",function(){		
	ShowModal("sys/aa/objecttype/index?suite_code="+$("#suite_code").val()+"&&module_code="+$("#module_code").val()+"&&?actioncallback=loadObjectType");
	
});


$("#btnModuleList").on("click",function(){	
	ShowModal("sys/aa/privilege/index?actioncallback=loadModuleList");
});

function loadModuleList(modList){ 
	event.preventDefault();
	$("#suite_id").val(modList.suiteId);
	$("#suite_code").val(modList.suiteCode); 
	$("#suite_name").val(modList.suiteFullName);
	$("#module_id").val(modList.id);	
	$("#module_code").val(modList.modCode);		
	$("#module_name").val(modList.modFullName);	
	
	$("#object_type_code").val("");
	$("#action_type_code").val("");
...
function loadObjectType(objActType){ 
	event.preventDefault();
	objActType = eval(objActType)
	$("#object_type_code").val(objActType[0]);
	$("#object_type_name").val(objActType[0]);	
	$("#action_type_code").val(objActType[1]);	
	$("#action_type_name").val(objActType[1]);
	$("#doa_type_field").val(objActType[2]);	
	$("#doa_limit_field").val(objActType[3]); 
	$("#object_type_id").val(objActType[4]);
	
	$("#workflow_code").val($("#module_code").val()+"_"+objActType[0]+"_"+objActType[1]);
	$("#workflow_name").val(objActType[1]+"_"+objActType[0]);
	$("#workflow_descr").val(objActType[1]+"_"+objActType[0]);
	
	$(".doa-type-text").html("( "+objActType[2]+" )");
	$(".doa-limit-text").html("( "+objActType[3]+" )");
	
	$(".doa-type-text-value").html(objActType[2]);
	$(".doa-limit-text-value").html(objActType[3]);
	
	$(".doa-type-value").val(objActType[2]);
	$(".doa-limit-value").val(objActType[3]);
	
	$(".doa-type-label-value").val(objActType[2]);
	$(".doa-limit-label-value").val(objActType[3]);
